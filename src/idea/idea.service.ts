...
import { getManager, Repository, getConnection, AfterInsert } from 'typeorm';
...
export class IdeaService {
  ...
  private async vote(idea: IdeaEntity, user: UserEntity, vote: Votes) {
    const transaction = getConnection().createQueryRunner()
    ...
    if (
      ...
      try{
        ...
        // 연동시키고
        // 오케이 떨어지면 트랜잭션 종료.
        await this.slackMsg.doAlarm('n번의 추천') // 로직 추가
        //OCR
        //Logic
        //translate(ocrResult)
        //translateRepository.save(translateResult);
